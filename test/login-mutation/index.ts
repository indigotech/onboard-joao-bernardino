...
describe('login', () => {
  ...
  it('should succeed if credentials are correct', async () => {
    ...
    const response = (await requestLogin(defaultLoginInput, serverUrl)).data;
  ...
  it('should extend token lifetime if remeberMe is true', async () => {
    ...
    const response = (await requestLogin({ ...defaultLoginInput, rememberMe: true }, serverUrl)).data;
    const loginInfo = response.data.login;
  ...
  it('should fail if no user has the given email', async () => {
    ...
    const response = (await requestLogin({ ...defaultLoginInput, email: 'wrong@email.com' }, serverUrl)).data;
  ...
  it('should fail if password is wrong', async () => {
    ...
    const response = (await requestLogin({ ...defaultLoginInput, password: 'wr0ngpassword' }, serverUrl)).data;
