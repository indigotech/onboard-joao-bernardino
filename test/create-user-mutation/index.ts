...
export function testCreateUserMutation(getServerUrl: () => string) {
  describe('createUser', () => {
    beforeEach(async () => {
    ...
    it('should create user in db and return it in response', async () => {
      ...
      const responseData = (await requestUserCreation(userInput, getServerUrl())).data.data.createUser;
    ...
    it('should give an error in case of short passwords', async () => {
      ...
      const responseData = (await requestUserCreation(userInput, getServerUrl())).data.errors[0];
